...
def index():
    ...
    columns= 'Total').sort_values('Total', ascending=False).reset_index().head()
    ...
    ## Bar Plot
    cat_order = df2.groupby('Category').agg({'Category' : 'count'}).rename({'Category':'Total'}, axis=1).sort_values('Total', ascending=False).reset_index().head()
